    ...
    <div class="mx-auto mt-8 max-w-4xl space-y-6 px-4 sm:px-6 lg:px-8">
        ...
        <form method="POST" action="{{ route('coupons.apply') }}" class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div>
                    <label for="cart_total" class="mb-2 block text-sm font-medium text-slate-700">Cart total (USD)</label>
            ...
            <div class="mt-4 flex flex-wrap gap-2">
                @forelse ($activeCoupons as $coupon)
                    <span class="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-medium text-slate-700">
                        {{ $coupon->code }} · {{ $coupon->type === 'percent' ? $coupon->value.'%' : 'USD '.number_format($coupon->value, 2) }}
                    </span>
                @empty
        ...
        @if (session('result'))
            ...
            <section class="rounded-2xl border border-indigo-200 bg-indigo-50 p-6 shadow-sm">
                ...
                <dl class="mt-4 grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
                    ...
                    <div><dt class="text-indigo-700/70">Cart total</dt><dd class="font-semibold text-indigo-900">USD {{ number_format($result['cart_total'], 2) }}</dd></div>
                    <div><dt class="text-indigo-700/70">Discount</dt><dd class="font-semibold text-indigo-900">USD {{ number_format($result['discount'], 2) }}</dd></div>
                    <div class="sm:col-span-2"><dt class="text-indigo-700/70">New total</dt><dd class="text-xl font-bold text-indigo-900">USD {{ number_format($result['new_total'], 2) }}</dd></div>
                </dl>
            </section>
        @endif
    </div>
