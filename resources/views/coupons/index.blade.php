                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($coupons as $coupon)
                            ...
                            <tr class="text-slate-700">
                                ...
                                <td class="px-4 py-3">{{ $coupon->type === 'percent' ? $coupon->value.'%' : 'USD '.number_format($coupon->value, 2) }}</td>
                                <td class="px-4 py-3 text-xs text-slate-600">
                                    {{ $coupon->start_date?->format('Y-m-d') ?? 'No start' }} → {{ $coupon->end_date?->format('Y-m-d') ?? 'No end' }}
                                </td>
                                <td class="px-4 py-3">{{ $coupon->min_order_amount ? 'USD '.number_format($coupon->min_order_amount, 2) : '-' }}</td>
